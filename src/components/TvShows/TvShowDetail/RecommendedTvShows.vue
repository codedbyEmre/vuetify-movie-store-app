<template>
  <RouterLink :to="`/tvShow-detail/${recommended.id}`" class="text-decoration-none">
    <v-card>
      <v-img
        v-if="recommended.poster_path"
        :src="`https://image.tmdb.org/t/p/original/${recommended.poster_path}`"
        :lazy-src="`https://image.tmdb.org/t/p/original/${recommended.poster_path}`"
        cover
        height="100%"
      >
      ...
      <div class="pa-3" :class="[!storeThemeSwitch.darkMode ? 'bg-blue-grey-darken-4' : 'white']">
        <div class="d-flex justify-space-between subtitle-2 mb-2">
          <div>
            {{ recommended?.first_air_date?.slice(0, 4) }}
          </div>
        </div>
...
const voteColor = computed(() => {
  const vote = props.recommended.vote_average.toFixed(1);
  if (vote >= 8.0) {
    return 'bg-green';
  } else if (vote >= 6.5) {
    return 'bg-yellow';
